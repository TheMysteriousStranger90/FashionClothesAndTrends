...
public static class SeedDataInitializer
{
    ...
    public static async Task SeedUsersAsync(UserManager<User> userManager, RoleManager<AppRole> roleManager)
    {
        ...
        {
            var roleName = role.Name ?? throw new InvalidOperationException("Role name is required.");
        ...
        foreach (var user in users)
        {
            if (await userManager.FindByEmailAsync(user.Email!) == null)
            {
                ...
                if (user.Email != null && user.Email.Contains("admin", StringComparison.OrdinalIgnoreCase))
                {
